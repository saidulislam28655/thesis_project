...
def DesignOpt(
    ...
    # thermal eval ekhon batch_XXXX subfolder-er bhitore thake (chunked solver
    # er karone), tai sob batch subfolder-er moddhe recursively peak_temp*.csv
    # khujte hobe. ekta filename -> full path map বানিয়ে rakhi.
    thermal_dir = os.path.join(os.getcwd(), path_thermal)
    thermal_file_map = {}
    for root, _, files in os.walk(thermal_dir):
        for f in files:
            if f.startswith("peak_temp") and f.endswith(".csv"):
                thermal_file_map[f] = os.path.join(root, f)
    ...
    for _, _, files in os.walk(directory):
        for file in files:
            if file.startswith("back_pressure") & file.endswith(".csv"):
                ...
                # read temperature (batch subfolder-e thakte pare, tai map theke khoji)
                thermal_filename = "peak_temp" + file[13:]
                if thermal_filename not in thermal_file_map:
                    raise FileNotFoundError(
                        f"{thermal_filename} pawa jay ni kono batch subfolder-e. "
                        f"thermal_dir checked: {thermal_dir}"
                    )
                with open(thermal_file_map[thermal_filename], "r") as datafile:
                    data = []
                    reader = csv.reader(datafile, delimiter=",")
                    for row in reader:
                        columns = [row[1]]
    ...
    opt_design_configs = np.array(
        [
            np.array(opt_design_configs[i][1:].replace("-checkpoint","").split("_")).astype(float)
